...
--동일한 SQL문장이란 : 텍스트가 완벽하게 동일한 SQL
--1.대소문자 가림
--2.공백도 동일해야함
--3.조회 결과가 같다고 동일한 SQL이 아님
--4.주석도 영향을 미침

--그허기 때문에 다음 SQL 문장은 동일한 문장이 아님
...
--위 두개의 SQL은 검색하고자 하는 부서번호만 다르고 나머지 테스트는 동일하다. 하지만 부서번호가 다르기 때문에 DBMS 입장에서는 서로 다른 SQL로 인식된다. 
--> 다른 SQL 실행 계획을 세운다.
--> 실행계획을 공유하지 못한다.
--> 해결책 바인드 변수
--SQL에서 변겨오디는 부분을 별도로 전송을 하고 실행계획이 세워진 이후에 바인딩 작업을 거쳐 
--실제 사용자가 원하는 변수 값으로 치환후 실행
--> 실행계획을 공유 --> 메모리 지원 낭비 방지
...
--SQL커서를 통해 개발자가 직접 데이터를 FETCH 함으로써 SELECT 결과를 전부 불러오지 않고도 개발이 가능
...
--부서 테이블을 활용하여 모든 행에 대해 부서번호 부서 이름을 CURSOR를 통해 
--FETCH, FETCH된 결과를 확인;
...
-- 명시적 커서를 ROF LOOP에서 사용할 수 있게끔 문법으로 제공
...
--인장가 있는 커서 선언 방법
    --CURSOR 커서이름(인자1 인자1타입 .....) IS
        --서브쿼리
        --(커서 선언시에 작성한 인자를 서브쿼리에서 사용 할 수 있다)
...
FROM
    (SELECT dt,
          LEAD (dt) OVER(ORDER BY dt DESC)dt_2,
          dt-LEAD (dt) OVER(ORDER BY dt DESC)a
    FROM dt);
